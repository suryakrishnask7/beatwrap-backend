  ...
  try {
    ...
    if (playlistId) {
      ...
      try {
        ...
        // Replace tracks completely (deprecated /tracks replaced with /items)
        await axios.put(`https://api.spotify.com/v1/playlists/${playlistId}/items`, 
          { uris: trackUris },
          { headers: { Authorization: `Bearer ${token}` } }
        );
      } catch (e) {
    ...
    if (!playlistId) {
      ...
      const createRes = await axios.post(`https://api.spotify.com/v1/me/playlists`, 
        { name: playlistName, description: 'Automatically generated by BeatWrap based on your daily listening.', public: false },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      playlistId = createRes.data.id;

      // Add tracks (deprecated /tracks replaced with /items)
